...
func main() {
    ...
    fmt.Println()
    fmt.Println(Str2DesEncrypt("sfe023f_"))
    fmt.Println()
    //Str2DesEncrypt2("NO8BxaR/BQ+Ga1iMX7M+tIS+dgUQkbZr", "sfe023f_")
}


func Str2DesEncrypt(value string) string {
    key := []byte(value)
...
func Str2DesEncrypt2(desValue string, key string) string {
    origData, err := DesDecrypt([]byte(desValue), []byte(key))
    if err != nil {
        panic(err)
    }
